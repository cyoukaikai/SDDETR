...
from smrc.object_tracking.cnf import IoUTracker, _MATCH_METRICS  # , OptFlowPCTracker
...
class AnnotateTracklet(AnnotateBBox, TrackerSMRC, Visualization):
    def __init__(self, image_dir, label_dir,
        ...
        self.auto_correct_class_label = False  # never auto correct class label, 2021-2-2 by Kaikai, Zhao

        self.num_pixels_bbox_to_extend_per_class = None
    ...
    def keyboard_listener(self, pressed_key, tmp_img=None):
        ...
        if pressed_key == 255 or pressed_key == 0 or pressed_key == -1:
            return
        print('pressed_key=', pressed_key, 'pressed_key & 0xFF =', pressed_key & 0xFF)
        if ord('a') <= pressed_key <= ord('z'):  # 'a': 97, 'z': 122
            ...
            # I did not set how to turn on self.play_music_on.
            ...
            # Viewing only a single cluster does not cope with annotation mode, i.e.,
            # if we draw something, it did not show.
            elif pressed_key == ord('t'): self.Event_RefreshTrackingResult(self.active_image_index)
            # self.Event_SwitchActiveObjectTrackbar()

            # failed, test: we can not distinguish the lower case and upper case
            # pressed_key= 106 pressed_key & 0xFF = 106  # j
            # pressed_key= 65505 pressed_key & 0xFF = 225  # shift key
            # pressed_key= 106 pressed_key & 0xFF = 106  # shift Key + J
            # elif pressed_key in [ord('k'), ord('K'), ord('j'), ord('J'),
            #                      ord('i'), ord('I'), ord('m'), ord('M')]:
            #     T = None
            #     if pressed_key == ord('k'): T = ['right', -self.move_unit]
            #     elif pressed_key == ord('K'): T = ['right', self.move_unit]
            #     elif pressed_key == ord('j'): T = ['left', self.move_unit]
            #     elif pressed_key == ord('J'):  T = ['left', -self.move_unit]
        ...
        # elif (pressed_key == ord('<') or pressed_key == ord('>')) and \
        #         len(self.clusters) > 1:
        #     if pressed_key == ord('<'): self.Event_MoveToPrevHole()
        #     elif pressed_key == ord('>'): self.Event_MoveToNextHole()
        ...
        elif pressed_key == self.keyboard['PAGEUP']: self.Event_MoveToActiveObjectStart()
        elif pressed_key == self.keyboard['PAGEDOWN']: self.Event_MoveToActiveObjectEnd()
        ...
        # elif pressed_key == self.keyboard['HOME']: self.Event_MoveBBoxWest()
        # elif pressed_key == self.keyboard['END']: self.Event_MoveBBoxEast()
        # elif pressed_key == self.keyboard['PAGEUP']: self.Event_MoveBBoxNorth()
        # elif pressed_key == self.keyboard['PAGEDOWN']: self.Event_MoveBBoxSouth()

        elif pressed_key == self.keyboard['SPACE']: self.Event_FinishActiveDirectoryAnnotation()
        elif pressed_key == ord(';'):  self.select_or_deselect_one_cluster()
        ...
        elif pressed_key == self.keyboard['TAB']:
            self.delete_unselected_object()
            # target_label = self.active_class_index
            # self.Event_ChangActiveObjectClassLabel(target_label)

        elif pressed_key == ord('['):  self.Event_MoveToActiveObjectStart()
        elif pressed_key == ord(']'):  self.Event_MoveToActiveObjectEnd()
        # elif pressed_key == ord(']'): self.Event_SelectActiveObject()
        elif pressed_key == ord('}'):  self.Event_DeleteObjectFirstHalf()
        elif pressed_key == ord('{'):  self.Event_DeleteObjectSecondHalf()
        ...
        # or pressed_key == ord('b')
        elif pressed_key == ord('@') or pressed_key == 91:
    ...
    def keyboard_listener_additional(self, pressed_key, tmp_img):
        ...
        # if self.keyboard_encoder.platform == 'Linux' and \
        #         ord('0') <= pressed_key <= ord('9'):
        #     if pressed_key == ord('7'):
        #         self.Event_MoveBBoxWest()
        #     elif pressed_key == ord('8'):
        #         self.Event_MoveBBoxEast()
        #     elif pressed_key == ord('9'):
        #         self.Event_MoveBBoxNorth()
        #     elif pressed_key == ord('0'):
        #         self.Event_MoveBBoxSouth()
        if self.keyboard_encoder.platform == 'Windows' and \
                ord('0') <= pressed_key <= ord('9'):
            if pressed_key == ord('7'):
                self.Event_MoveToPrevImage()
            # elif pressed_key == ord('8'): self.Event_MoveBBoxEast()
            elif pressed_key == ord('9'):
                self.Event_MoveToNextImage()
            # elif pressed_key == ord('0'): self.Event_MoveBBoxSouth()

        # Currently we have 6 classes, if the number of classes increased, we should modify the
        # following lines.
        elif pressed_key in [ord('!'), ord('"'), ord('#'), ord('$'), ord('%'), ord('&')]:
            target_label = pressed_key - ord('!')
            self.Event_ChangActiveObjectClassLabel(target_label)
        elif pressed_key == self.keyboard['F1']:
            target_label = self.active_class_index
            self.Event_ChangActiveObjectClassLabel(target_label)
    ...
    def Event_SwitchFittingMode(self):
        """
        # turn on or off the fitting mode
        :return:
        """
        super().Event_SwitchFittingMode()
        if not self.fitting_mode_on:  # fitting mode already on
            self.Event_RefreshTrackingResult(self.active_image_index)

        # if self.fitting_mode_on:  # fitting mode already on
        #     # reinitialize the curve_fitting_dict
        #     self.curve_fit_manually()
        #     self.last_fitted_bbox = self.fitted_bbox.copy()
    ...
    def Event_ChangActiveObjectClassLabel(self, target_label):
    ...
    @staticmethod
    def object_tracking(video_annotation_list):
    ...
    def tracking_main(self):
        ...
        def init_empty_frame_det():
            self.frame_dets = {}
            for image_path in self.IMAGE_PATH_LIST:
                self.frame_dets[image_path] = []
        ...
        if num_bbox == 0:
            ...
            my_tracker = self.object_tracking(video_annotation_list)
            ...
            # ===============================================================
            # if self.auto_correct_class_label:
            #     pass  # never auto correct class label, 2021-2-2 by Kaikai, Zhao
                ...
                if active_bbox is not None:
                    ...
                    if global_bbox_id is not None:
                        ...
                        if object_id_to_display is not None:
                            ...
                            #         )
                            #         cluster_hole = sorted(list(set(
                            #             range(image_id_list_sorted[0], image_id_list_sorted[-1])
                            #         ) - set(image_id_list_sorted)))
                            #         if len(cluster_hole) > 0:
                            #             text_content += 'FN: ['
                            #             max_display_ind = min(num_holes - 1, 5)
                            #             # text_content +=  ', '.join(map(str, cluster_hole[:])) + '\n'
                            #
                            #             for x in range(max_display_ind):
                            #                 # print(f'x={x}, {display_list[10*x:10*(x+1)]}')
                            #                 text_content += str(cluster_hole[x]) + ', '
                            #
                            #             if num_holes > max_display_ind+1:
                            #                 text_content += str(cluster_hole[max_display_ind]) + ' ... ]\n'
                            #             else:
                            #                 text_content += str(cluster_hole[max_display_ind]) + ']\n'
                            # num_holes = self.get_number_of_hole_in_cluster(self.clusters[active_cluster_id])
                            # if num_holes > 0:
    ...
    def draw_bbox_with_tracking_results(self, tmp_img, bbox, global_bbox_id):
        ...
        if object_id_to_display is not None:
            if original_class_idx != corrected_class_idx:
                ...
                # Draw here does not make much sense as draw_active_bbox may change the color to yellow any way
    ...
    def draw_bboxes_from_file(self, tmp_img, ann_path):
        ...
        # image_path should always in self.frame_dets even it is empty.
        # self.frame_dets is initialized in self.tracking_main()
        assert image_path in self.frame_dets
    ...
    def draw_bboxes_from_file_additional_operation(self, tmp_img):
        for bbox in self.active_image_annotated_bboxes:
            ...
            if cluster_id is not None and (0 <= cluster_id <= len(self.clusters)):
                holes = self.get_image_ids_for_hole_in_cluster(self.clusters[cluster_id])
                num_holes = len(holes)
                if num_holes > 0:
                    # class_color = self.CLASS_BGR_COLORS[class_idx].tolist()
                    text_content = f'{num_holes} FN'  # [{holes}
                    LINE_THICKNESS = 2
                    ...
                    #                f'{"%.1f" % distances[1]}, ' \
                    #                f'{"%.1f" % distances[2]}'
                    # cv2.putText(tmp_img, text_content, (x1, y2 + text_height + margin),
                    #             font, font_scale, smrc.not_used.RED,
                    #             self.LINE_THICKNESS,
                    #             int(cv2.LINE_AA))
                    self.draw_class_name(tmp_img,
                                         (x1, y2 + text_height + margin),
                                         text_content,
                                         smrc.utils.RED, text_color=(0, 0, 0)
                                         )  #
            ...
            for cluster_id in self.cluster_IDs_selected:
                ...
                if self.active_image_index in image_ids:
                    ...
                    if bbox in self.active_image_annotated_bboxes:
                        ...
                        # )
                        self.draw_single_bbox(tmp_img, bbox,
                                              smrc.utils.color.Orange,
                                              smrc.utils.color.Orange, smrc.utils.color.Orange)
    ...
    def delete_unselected_object(self):
        ...
        if len(self.cluster_IDs_selected) > 0:
            ...
            # self.deleted_bbox_history.append(deleted_dict)
            # ========================= slow version over
            print(f'Deleting unselected bbox, it may take time, please wait for a while ...')
            self.Event_DeleteAllBBoxForActiveDirectory()
    ...
    # def Event_MoveToPrevHole(self):
    #     self.move_to_prev_or_next_hole(prev_or_next='prev')
    #
    # def move_to_prev_or_next_hole(self, prev_or_next):
    #     assert prev_or_next in ['prev', 'next']
    #
    #     if self.active_cluster_id is not None and self.clusters is not None \
    #             and self.active_cluster_id < len(self.clusters):
    #         image_id_list_sorted = self.get_image_id_list_sorted(
    #             self.clusters[self.active_cluster_id]
    #         )
    #         cluster_hole = list(set(
    #             range(image_id_list_sorted[0], image_id_list_sorted[-1])
    #         ) - set(image_id_list_sorted))
    #         if len(cluster_hole) > 1:
    #             cur_image_id = self.active_image_index
    #             next_image_id = None
    #             if prev_or_next == 'next':
    #                 for image_id in cluster_hole:
    #                     if cur_image_id < image_id:
    #                         next_image_id = image_id
    #                         break
    #             else:
    #                 for image_id in cluster_hole[::-1]:
    #                     if cur_image_id > image_id:
    #                         next_image_id = image_id
    #                         break
    #             if next_image_id is not None:
    #                 self.set_image_index(next_image_id)
    #                 # cv2.setTrackbarPos(self.TRACKBAR_IMG, self.IMAGE_WINDOW_NAME, self.active_image_index)
    #                 self.set_active_bbox_after_set_active_cluster_id()
    #
    # def Event_MoveToNextHole(self):
    #     self.move_to_prev_or_next_hole(prev_or_next='next')

    def Event_DeleteObjectFirstHalf(self):
        if self.active_cluster_id is not None and self.clusters is not None \
                and self.active_cluster_id < len(self.clusters):
            active_cluster = self.clusters[self.active_cluster_id]
            to_remove_global_bbox_ids = [
                x for x in active_cluster if self.get_image_id(x) <= self.active_image_index
            ]
            self.delete_one_cluster(to_remove_global_bbox_ids)

    def Event_DeleteObjectSecondHalf(self):
        if self.active_cluster_id is not None and self.clusters is not None \
                and self.active_cluster_id < len(self.clusters):
            active_cluster = self.clusters[self.active_cluster_id]
            to_remove_global_bbox_ids = [
...
class AnnotateTrackletDeprecated(AnnotateTracklet):
    def __init__(self, image_dir, label_dir,
                 class_list_file, user_name=None
                ):
        super(AnnotateTracklet, self).__init__(
            image_dir, label_dir,
            class_list_file, user_name=user_name)
    ...
    def draw_bboxes_from_file(self, tmp_img, ann_path):
        ...
        # visualize only the active cluster
        if self.show_only_active_cluster_on:
            # len(self.clusters) == 0 or image_path not in self.frame_dets or
            if len(self.frame_dets[image_path]) == 0:
                return
            for idx, global_bbox_id in enumerate(
                    self.frame_dets[image_path]):  # 0 0.512208657048 0.455160744501 0.327413984462 0.365482233503
                image_id, bbox = self.get_image_id_and_bbox(global_bbox_id)
