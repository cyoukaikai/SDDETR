...
class DetectionToAnnotation(Tracking):
    ...
    def preprocess_json_detection_for_annotation(
        ...
        # one image, one object_detection list (empty in the case of no any object_detection result)
...
#             iou = smrc.not_used.compute_iou(pred[1:5], ground_truth_bbox[1:5])
...
#                       f'[eval_old]{"{0:.4f}".format(eval_annotation)}')
#             # 'hi'.ljust(10)
...
#     print(f'Sorting result based on scores, {len(pred_flattened)} predicted bboxes...')
...
#     # smrc.not_used.save_1d_list_to_file('tp_list.txt',TP_all )
#     # print(f'fp in total = {np.sum(FP_all)}')
#     # smrc.not_used.save_1d_list_to_file('fp_list.txt',FP_all )
#     # TP_accumulated_tmp = np.cumsum(TP_all)
#     # smrc.not_used.save_1d_list_to_file('TP_accumulated_tmp.txt',TP_accumulated_tmp )
#     # print(f'fn in total = {num_ground_truth_bbox - np.sum(TP_all)}')
#
#     # sys.exit(0)
#     print('Estimating precision_values, recall_values for the sorted result ...')
...
#     ann_path_list = smrc.not_used.get_file_list_recursively(label_dir)
#     prediction_all = []
...
#     prediction_all = []
#     for ann_path in ann_path_list:
#         object_detection = smrc.not_used.load_bbox_from_file(ann_path)
#         # each file corresponding to one element in prediction_all
#         prediction_all.append(object_detection)
#
#     return prediction_all
...
#        class_name_list = list(smrc.not_used.non_blank_lines(f))
...
#         image_dir, dir_name, image_name = smrc.not_used.split_image_path(image_path)
#
