...
class AnnotateSparseBBox(AnnotateBBox):
    def __init__(self, image_dir, label_dir, class_list_file,
        ...
        self.IMAGE_WINDOW_NAME = 'SparseBBoxAnnotationTool'
    ...
    def keyboard_listener(self, pressed_key, tmp_img=None):
        ...
        if pressed_key == ord(','): self.Event_MoveToPrevDetection(tmp_img)
    ...
    def load_all_detection(self):
        ...
        for image_id, img_path in pbar:
            pbar.set_description(f'Loading detections for '
                                 f'{image_id}/{len(self.IMAGE_PATH_LIST)} th image')
...
class AnnotateSparseBBoxDeprecated(AnnotateSparseBBox):
    def __init__(self, image_dir, label_dir, class_list_file,
                 user_name=None, blur_bbox=False, music_on=False
                 ):
