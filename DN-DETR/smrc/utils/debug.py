...
import os
import numpy as np
from .file_path import generate_dir_if_not_exist
...
def merging_two_images_top_by_down(img1, img2):
    """
    convert two images to one side by side
    modified from
    https://stackoverflow.com/questions/7589012/combining-two-images-with-opencv
    ...
    W = max(w1, w2)
    # Create an array big enough to hold both images next to each other.
    img = np.zeros((h1 + h2, W, 3), np.uint8)

    # Copy both images into the composite image.
    # dim = (W, a_h)
    img[:h1, :, :] = cv2.resize(img1, (W, h1))
    img[h1: h1 + h2, :, :] = cv2.resize(img2, (W, h2))
    # cv2.imshow('tests', img)
    return img
