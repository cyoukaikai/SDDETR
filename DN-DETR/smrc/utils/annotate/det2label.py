######################################################################
# import object_detection result to the annotation tool
...
import os
import smrc.utils
import smrc.utils.det.yolo_json
...
class ImportDetection:
    ...
    # The imported result will be put in label_dir (default dir, 'labels')
    def __init__(self,
        ...
        self.import_detection_to_annotation_tool(dir_list)
    ...
    def import_detection_to_annotation_tool(self, dir_list=None):
        # load only the directory name not the full path (True)
        # json_file_list = smrc.not_used.get_smrc_json_file_list(
        ...
        if dir_list is not None:
            json_file_list = [json_file for json_file in json_file_list
                              if utils.det.yolo_json.extract_smrc_json_dir(json_file) in dir_list]
        ...
        # dir_list = smrc.not_used.get_json_dir_list(self.json_file_dir)
        for dir_idx, json_file_name in enumerate(json_file_list):
            dir_name = smrc.utils.det.yolo_json.extract_smrc_json_dir(json_file_name)
            ...
            detection_dict = smrc.utils.det.yolo_json.load_yolov3_json_detection_to_dict(
