...
class ImageSequenceViewer:
    ...
    def visualization_annotation(self, dir_list, result_dir=None, generate_video=False, ext_str='.mp4', fps=30):
        ...
        for dir_idx, dir_name in pbar:
            ...
            if generate_video:
                pathIn = os.path.join(result_dir, self.active_directory)
                pathOut = pathIn + ext_str  # truck_images/1 -> truck_images/1.mp4
                smrc.utils.convert_frames_to_video(pathIn, pathOut, fps=fps)

        # if generate_video:
        #     smrc.utils.convert_frames_to_video_inside_directory(
        #         result_dir, fps=30, ext_str='.mp4')
