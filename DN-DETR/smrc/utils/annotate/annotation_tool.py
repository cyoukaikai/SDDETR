...
class Draw:
    def __init__(self):
        ...
        self.BBOX_WIDTH_OR_HEIGHT_THRESHOLD = 10
        # to decide the smallest bbox width or height we allowed to draw. Using a negative value will disable
        # this limitation
    ...
    def draw_single_bbox(self, tmp_img, bbox, rectangle_color=None,
        ...
        if smrc.utils.bbox_area(bbox) > self.bbox_area_threshold_for_thick_line:
            thickness = 2

        self.draw_bbox_with_specialized_shape(
            tmp_img=tmp_img, bbox=bbox, color=rectangle_color,
            anchor_rect_color=anchor_rect_color, thickness=thickness
        )
        ...
        self.draw_class_name(tmp_img, (xmin, ymin), caption, text_shadow_color, text_color)  #

    def draw_bbox_with_specialized_shape(self, tmp_img, bbox, color,
                                         thickness, **kwargs):
        class_idx, xmin, ymin, xmax, ymax = bbox
        ...
        self.draw_bbox_anchors(tmp_img, xmin, ymin, xmax, ymax,
                               anchor_color=anchor_color, line_thickness=thickness
                               )
    ...
    def draw_annotated_bbox(self, tmp_img, bbox):
        # equal to self.draw_single_bbox(tmp_img=tmp_img, bbox=bbox)
...
class MainWindow(Color, ActiveImage, Draw, BboxAnchor):
    ...
    def init_class_list_and_class_color(self):
        assert os.path.isfile(self.class_list_file),\
            'File {} not exist, please check.'.format(self.class_list_file)
        # load class list
    ...
    def set_active_bbox_idx_based_on_mouse_position(self, allow_none=True):
        ...
        for idx, obj in enumerate(self.active_image_annotated_bboxes):
            _, x1, y1, x2, y2 = obj
            xmin, ymin, xmax, ymax = self.get_bbox_active_region_rectangle(
                * smrc.utils.get_min_rect(x1, y1, x2, y2)
                # in case the points are not top left, bottom right format.
            )
    ...
    def draw_bboxes_from_file(self, tmp_img, ann_path):
        ...
        '''
        self.active_image_annotated_bboxes = smrc.utils.load_bbox_from_file(ann_path)
...
class AnnotationTool(MainWindow):
    def __init__(self, image_dir=None, label_dir=None, class_list_file=None):
        ...
        # the directory where the final annotation result are moved to
        ...
        if self.class_list_file is not None:
            assert os.path.isfile(self.class_list_file)
    ...
    def delete_active_bbox(self):
        ...
        # smrc.not_used.save_bbox_to_file(ann_path, self.active_image_annotated_bboxes)
    ...
    def add_single_bbox(self, ann_path, bbox):
        ...
        if bbox in self.active_image_annotated_bboxes:
            ...
            # txt_line = smrc.not_used.bbox_transfer_to_txt_line_format(class_idx, xmin, ymin, xmax, ymax)
...
class AnnotationToolDeprecated(AnnotationTool):
    ...
    def delete_active_bbox_non_max_suppression(compress_thd):
        ...
        #             print([x1, y1, x2, y2])
        #             print('iou = ', smrc.not_used.compute_iou(active_bbox_rect, [x1, y1, x2, y2]))
        #             if smrc.not_used.compute_iou(active_bbox_rect, [x1, y1, x2, y2]) > compress_thd:
