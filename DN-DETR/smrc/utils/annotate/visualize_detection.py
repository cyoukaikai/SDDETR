...
# from smrc.not_used.det.yolov3_smrc_json import load_yolov3_json_detection_to_dict
# from smrc.not_used.det.yolov4_json import load_yolov4_json_detection_to_dict
...
class VisualizeDetection(VisualizeLabel):
    """
    Currently, this class only support json format detection.
    """
    def __init__(self, image_dir,
                 class_list_file, json_file_dir=None,
                 label_dir=None, auto_load_directory=None,
                 detection_format='yolov3',  # 'yolov3'
                 score_thd=10, nms_thd=50,
                 user_name=None,
                 max_img_num=None
                 ):
    ...
    def load_active_directory_additional_operation(self):
        ...
        # load all the detections for visualization
        self.active_directory_detection_dict = load_yolo_json_det(
            json_detection_file=json_detection_file,
            score_thd=None, nms_thd=None, det_format=self.detection_format_,
    ...
    @staticmethod
    def get_last_two_level_path(image_path_full):
    ...
    def draw_bboxes_from_file(self, tmp_img, score_thd):
        ...
        image_path = self.get_last_two_level_path(image_path_full)
    ...
    def visualize_results(self):
        ...
        # limit the image to view
        if self.max_img_num is not None and isinstance(self.max_img_num, int):
            self.IMAGE_PATH_LIST = self.IMAGE_PATH_LIST[:self.max_img_num]
            self.LAST_IMAGE_INDEX = len(self.IMAGE_PATH_LIST) - 1
            
            image_paths = [self.get_last_two_level_path(image_path_full) for image_path_full in self.IMAGE_PATH_LIST]
            keys_to_delete = []
            for img_path in self.active_directory_detection_dict.keys():
                if img_path not in image_paths:
                    keys_to_delete.append(img_path)
            for key in keys_to_delete:
                del self.active_directory_detection_dict[key]
