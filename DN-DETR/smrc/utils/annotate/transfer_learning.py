...
def generate_training_data_image_list_file(
    ...
    pbar = tqdm(enumerate(training_directory_list))
    for dir_idx, dir_name in pbar:
        dir_path = os.path.join(label_dir, dir_name)
        pbar.set_description('Processing directory: {}/{}, {}'.format(str(dir_idx + 1),
        ...
        for file_idx, image_path in enumerate(image_path_list):
            ...
            if os.path.isfile(image_path):  # cv2.imread(image_path) is not None:
                ...
                if os.path.isfile(ann_path):
                    ...
                    bbox_list = load_bbox_from_txt_file(ann_path, label_format)
                    if len(bbox_list) > 0:
...
# ):
#     if video_list is None:
#         dir_list = smrc.not_used.get_dir_list_in_directory(label_dir)
#     else:
#         dir_list = video_list
#     print(f'To load {len(dir_list)} directories.')
#
#     smrc.not_used.generate_dir_if_not_exist(result_root_dir)
...
#         video_non_empty_image_list = []
#         ann_path_list = smrc.not_used.get_file_list_in_directory(ann_dir_path)
#         for file_idx, ann_path in enumerate(ann_path_list):
#             bbox_list = smrc.not_used.load_bbox_from_file(os.path.abspath(ann_path))
#
#             image_path = smrc.not_used.get_image_or_annotation_path(ann_path, label_dir, image_dir, '.jpg')
#             if len(bbox_list) > 0:
...
#         save_image_list_to_file_incrementally(file_list_non_empty_image, video_non_empty_image_list)
#
#     training_image_list = smrc.not_used.load_1d_list_from_file(file_list_non_empty_image)
#     random.shuffle(training_image_list)
#
...
#
#     smrc.not_used.generate_dir_if_not_exist(resulting_image_dir)
#     smrc.not_used.generate_dir_if_not_exist(resulting_label_dir)
#
#     image_path_list_final = []
#     for i, image_path in enumerate(training_image_list):
#         ann_path = smrc.not_used.get_image_or_annotation_path(image_path, image_dir, label_dir, '.txt')
#         new_image_path = os.path.join(resulting_image_dir, image_path.replace(os.path.sep, '_'))
#
#         new_ann_path = smrc.not_used.get_image_or_annotation_path(
#             new_image_path,
#             resulting_image_dir,
...
#     )
#     smrc.not_used.save_1d_list_to_file(resulting_file, image_path_list_final)
...
class TransferLearning:
    ...
    def balanced_data(
        ...
        for dir_idx, dir_name in pbar:
            ...
            image_path_list = smrc.utils.get_image_file_list_in_directory(
        ...
        for class_id in sorted_class_ids:
            ...
            smrc.utils.save_1d_list_to_file(result_file_name, image_path_list)
...
def prepare_yolov4_label(
    ...
    smrc.utils.data_set.remove_txt_files(image_root_dir)

    # copy the txt files to image dir
    smrc.utils.data_set.copy_labels(
        source_label_dir=yolo_label_root_dir,
        target_label_dir=image_root_dir
    )
    # padding empty files
    if padding_empty_file_flag:
        smrc.utils.data_set.add_empty_txt_files(image_root_dir=image_root_dir)