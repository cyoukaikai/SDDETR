...
class AnnotationPostProcess:
    def __init__(self,
                 ...
                 min_bbox_width=1, min_bbox_height=1, min_area=1
                 ):
        ...
        print('====================================================')
        print(f'To check and correct annotation result in {self.label_dir_to_check}.')
        print(f'The images in {self.image_dir} will be used to assist the checking.')
        print(f'The corrected result will be saved in {self.checked_result_dir}.')
    ...
    def check_and_correct_annotation_results(
            self, label_dir_to_check, checked_result_dir, directory_list=None,
            min_bbox_width=1, min_bbox_height=1, min_area=1
    ):
        """Import the annotation result and conduct checking and correcting operations
        # input:
        ...
        assert len(directory_list) > 0, f'The annotation result in ' \
                ...
                if test_img is not None:
                    ...
                    for bbox_idx, bbox in enumerate(annotated_bboxes):
                        ...
                        if len(self.class_index_list) > 0 and not self.is_valid_class_idx(class_idx):
                            ...
                            continue
                        elif xmax - xmin < min_bbox_width or ymax - ymin < min_bbox_height \
                                or smrc.utils.bbox_area(bbox) < min_area:
                            print(f'xmin = {xmin}, xmax = {xmax}, ymin = {ymin}, ymax = {ymax}, '
                                  f'bbox_width = {xmax - xmin}, bbox_height = {ymax - ymin}, '
                                  f'bbox_area = {smrc.utils.bbox_area(bbox)}, deleted ...')
                            continue
