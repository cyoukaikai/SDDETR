...
from scipy.interpolate import UnivariateSpline, PPoly  # PiecewisePolynomial
import sys
import os
import cv2
...
def fit_piecewise_smoothing(x, y, smoothing_factor=0.05):
    """
    x, y below will cause problem, why and how to avoid the problem?
    x = [213, 214, 215, 216, 217, 219, 220]
    y = [41, 37, 33, 38, 36, 42, 43]
    v = np.std(y)
    v = 3.3319725113401715
    ...
    # and more importantly, the fitting result are determined.
    # k = len(y) - 1 if len(y) <= 3 else 3  # default 3, is vary dangerous.

    k = len(y) - 1 if len(y) <= 3 else 2
...
def annotation_fit_piecewise_smoothing(x, y, smoothing_factor=0):
    k = 1
    spl = UnivariateSpline(x, y, k=k)
    spl.set_smoothing_factor(smoothing_factor)
    return spl
...
def fit_and_fill(x_fit, y_fit, x_fill, k=1):
    # print(x_fit)
    # print(y_fit)
    assert k in [1, 'AUTO']
    if k == 'AUTO':
        spl = fit_piecewise_smoothing(x=x_fit, y=y_fit)
    else:
        spl = annotation_fit_piecewise_smoothing(x=x_fit, y=y_fit)
...
def post_process_fitted_bbox_rect(xmin, ymin, xmax, ymax, image_width, image_height):
    """
    modified from smrc.not_used.post_process_bbox_coordinate
    :param xmin:
    :param ymin:
    :param xmax:
    :param ymax:
...
class CurveFitting:
    ...
    def fit_and_fill_bbox_list(
            X1, X2, X3, X4,
            image_id_list, image_id_to_fill,
            debug=False, plot_option=False, k=1  # True
    ):
        """
        ...
        X1_fitted, spl_X1 = fit_and_fill(t, X1, image_id_to_fill, k=k)
        X2_fitted, spl_X2 = fit_and_fill(t, X2, image_id_to_fill, k=k)
        X3_fitted, spl_X3 = fit_and_fill(t, X3, image_id_to_fill, k=k)
        X4_fitted, spl_X4 = fit_and_fill(t, X4, image_id_to_fill, k=k)
...
class BBoxCurveFitting(CurveFitting):
    ...
    def save_fitted_bbox_for_checking(self, original_bbox_data, fitted_data, result_root_dir='fitting'):
        ...
        # smrc.not_used.generate_dir_if_not_exist(
        ...
        for image_id, bbox in bbox_list_all_none_empty:
            ...
            # ann_path = smrc.not_used.get_image_or_annotation_path(
    ...
    #
    #     image_path = self.IMAGE_PATH_LIST[0]
    #     image_dir, video_dir, _ = smrc.not_used.split_image_path(image_path)
    #     smrc.not_used.generate_dir_if_not_exist(
    #         os.path.join(result_root_dir, video_dir)
    #     )
    #
    ...
    #         # =================================================
    #         # the bbox could be None
    #         smrc.not_used.save_bbox_to_file(ann_path, [bbox])
    #         # =================================================
    #
    #         tmp_img = cv2.imread(image_path)
    #         annotation_tool.draw_bboxes_from_file(tmp_img, ann_path)
...
class Det2LabelCurveFitting(BBoxCurveFitting):
    def __init__(self, image_path_list, image_dir, label_dir):
        super().__init__(image_path_list, image_dir, label_dir)
        self.IMAGE_DIR = image_dir
        self.LABEL_DIR = label_dir
