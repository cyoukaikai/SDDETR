...
######################################################################
# handle the result of yolo based object_detection of the smrc json format
######################################################################
...
# def suppress_one_frame_det_by_another(frame_a_det, frame_b_det, nms_thd):
#     """
#     Suppressing the detections in frame_a_det by using the detections in frame_b_det,
#     here
#     :param frame_a_det:
#     :param frame_b_det:
#     :param nms_thd:
...
#     """
#     result = []
#     if len(frame_a_det) > 0:
#         # if noise detected
#         if len(frame_b_det) > 0:
#             for det in frame_b_det:
#                 # the last location is the score
#                 det[-1] = 1.0
#
#         result = non_max_suppression_single_image(
#             frame_a_det.extend(frame_b_det), nms_thd
#         )
#     return result


def suppress_noise_det(detection_dict, noise_det_dict, nms_thd):
    print('========================================================')

    # We first conduct nms independently for each of the det_dict (the det of the a video)
    print('Conducting the nms for the source detections')
    detection_dict = nms_detection_dict(detection_dict, nms_thd)

    print('conduct the nms for the noise detections with large threshold 0.5')
    noise_det_dict = nms_detection_dict(noise_det_dict, 0.5)

    final_detection_dict = {}
    removed_noise_dict = {}
    det_num = count_det_num(detection_dict)
    for image_path in detection_dict:
        # result = suppress_one_frame_det_by_another(
        #     detection_dict[image_path],
        #     noise_det_dict[image_path],
        #     nms_thd
        # )
        if image_path in detection_dict and len(detection_dict[image_path]) > 0:
            # if noise detected
            if image_path in noise_det_dict and len(noise_det_dict[image_path]) > 0:
                # for remaining noise detections
                for det in noise_det_dict[image_path]:
                    # the last location is the score
                    det[-1] += 1.0
                    # make the score higher than 1.0 to make sure the noise dets are not be
                    # suppressed by source detections.

                detection_to_suppress = detection_dict[image_path] + noise_det_dict[image_path]
                final_detection_dict[image_path] = non_max_suppression_single_image(
                    detection_to_suppress, nms_thd
                )
                # remove the noise detections from the final result
                for det in noise_det_dict[image_path]:
                    # the noise detection can also be removed due to nms inside the noise detections
                    if det in final_detection_dict[image_path]:
                        final_detection_dict[image_path].remove(det)

                # save the removed noise detection
                removed_noise_dets = [
                    det for det in detection_dict[image_path]
                    if det not in final_detection_dict[image_path]
                ]
                if len(removed_noise_dets) > 0:
                    removed_noise_dict[image_path] = removed_noise_dets
            else:
                final_detection_dict[image_path] = detection_dict[image_path]

    det_num_after_nms = count_det_num(detection_dict)
    print(f'{det_num - det_num_after_nms} detections suppressed by noise detection with nms_thd {nms_thd}, '
          f'remaining {det_num_after_nms} detections ...')
    return final_detection_dict, removed_noise_dict
