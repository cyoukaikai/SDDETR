...
from .color import *
from .file_path import natural_sort_key, get_file_list_recursively, get_dir_list_in_directory, \
    get_file_list_in_directory, \
    generate_dir_if_not_exist, append_prefix_to_file_path
...
def get_image_size_for_image_sequence(image_sequence_dir):
    """Get the image size for a video (a list of image_sequence)

    :param image_sequence_dir:
    :return: height, width of the first image in the image sequence
    """
...
def convert_frames_to_video_with_cut_background(background_image_filename, pathIn, pathOut, fps):
    ...
    files = get_image_file_list_in_directory(pathIn)
    ...
    if len(files) > 1:
        ...
        print('There are only %d image frames in %s, failed to generate video.' % (len(files), pathIn))


def convert_frames_to_video(pathIn, pathOut, fps, ext_str: str = ''):
    # frame_array = []
    # 'visualization/image/2595/0000.jpg'

    files = get_file_list_in_directory(pathIn, ext_str=ext_str)
...
def convert_frames_to_video_for_given_image_list(image_list, pathOut, fps, frames_per_video=None):
    assert pathOut.find('.') > -1

    height_width_list = []
    for filename in image_list:
        height_width_list.append([get_image_size(filename)])
    max_height, max_width = np.max(np.array(height_width_list), axis=0)[0]  # [[720, 1280]]
    ...
    prefix, ext = os.path.abspath(pathOut).rpartition('.')[-3], \
                  os.path.abspath(pathOut).rpartition('.')[-1]

    num_videos = int(len(image_list) / (frames_per_video * fps)) + 1
    print('num_videos = %d' % (num_videos,))
    pbar = tqdm(enumerate(range(num_videos)))
    for k, video_id in pbar:  # frames_per_video sec, iamge frame length for this video
        ...
        pbar.set_description('Processing video %s ' % video_name)
        ...
        if len(files_to_process) > 1:
            ...
            print('There are only %d image frames, failed to generate video.' % (len(files_to_process)))
        out.release()
...
def convert_frames_to_video_for_all_images_in_parent_dir(pathIn, pathOut, fps, frames_per_video,
                                                         shuffle_option=False):
    # image_list = get_file_list_recursively(pathIn, ext_str='.jpg')
    image_list = [file for file in get_file_list_recursively(pathIn) if is_image(file)]
    ...
    num_batch = int(np.ceil(len(image_list) / batch_size))
    for k in range(num_batch):
        tmp_pathOut = append_prefix_to_file_path(pathOut, f'batch_{k}')
        convert_frames_to_video_for_given_image_list(
            image_list[k * batch_size: min(len(image_list), (k+1) * batch_size)],
            tmp_pathOut, fps,
            frames_per_video)
