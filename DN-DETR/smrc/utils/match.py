...
# Hungarian method
# class Match:
def HungarianMatch(dist_matrix, max_distance=None):
    """
    >>> import numpy as np
    >>> from scipy.optimize import linear_sum_assignment
    ...
    if max_distance is not None:
        for k, (row_ind, col_ind) in enumerate(zip(row_inds, col_inds)):
            if dist_matrix[row_ind, col_ind] > max_distance:
                row_inds[k], col_inds[k] = -1, -1
...
def BidirectionalMatch(
    ...
    row_inds = np.array(range(num_pre_det))
    col_inds = np.array([-1] * num_pre_det) # use -1 to represent the invalid matching
    for i in range(num_pre_det):
...
def _filter_out_none_matching(row_inds, col_inds):
    ...
    valid_ids = np.not_equal(col_inds, -1)
...
def AHCMatch(dist_matrix, max_distance):
    ...
    # transfer cluster._label to matching result
    num_pre_det = dist_matrix.shape[0]
    row_inds = np.array(range(num_pre_det))
    col_inds = np.array([-1] * num_pre_det)  # use -1 to represent the invalid matching
