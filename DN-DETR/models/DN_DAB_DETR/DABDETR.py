...
def sigmoid_focal_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2):
    ...
    Returns:
        Loss tensor
    """
    prob = inputs.sigmoid()  # torch.Size([2, 300, 91])
...
class DABDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        self.label_enc = nn.Embedding(num_classes + 1, hidden_dim - 1)  # TODO: what is label encoding?
        self.num_classes = num_classes

        if bbox_embed_diff_each_layer:
            self.bbox_embed = nn.ModuleList([MLP(hidden_dim, hidden_dim, 4, 3) for i in range(6)])
        else:
            self.bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3)  # 4: output channels, 3: number of  layers,
        ...
        self.refpoint_embed = nn.Embedding(num_queries, query_dim)   # object query
        self.random_refpoints_xy = random_refpoints_xy
        if random_refpoints_xy:
            # import ipdb; ipdb.set_trace()
            self.refpoint_embed.weight.data[:, :2].uniform_(0,1)
            self.refpoint_embed.weight.data[:, :2] = inverse_sigmoid(self.refpoint_embed.weight.data[:, :2])
            self.refpoint_embed.weight.data[:, :2].requires_grad = False  # TODO: can we freeze a part of the tensor
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        src_logits = outputs['pred_logits'] # torch.Size([2, 300, 91])

        idx = self._get_src_permutation_idx(indices)
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)])
        target_classes = torch.full(src_logits.shape[:2], self.num_classes,
                                    dtype=torch.int64, device=src_logits.device)  # torch.Size([2, 300])
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes'][idx]  # (22, 4)
