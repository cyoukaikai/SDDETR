...
def prepare_for_dn(dn_args, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    Args:
        ...
        label_enc: label encoding embedding  Embedding(92, 255)
    ...
    if num_patterns == 0:
        num_patterns = 1
    indicator0 = torch.zeros([num_queries * num_patterns, 1]).cuda()  # torch.Size([300, 1]) if there is problem, check dn_args training or eval
    # torch.Size([300, 255]) num_classes here means the background, so the labels are initialized as background.
    tgt = label_enc(torch.tensor(num_classes).cuda()).repeat(num_queries * num_patterns, 1)
    tgt = torch.cat([tgt, indicator0], dim=1)  # torch.Size([300, 256]), last dimension means if the box is gt or not.
    refpoint_emb = embedweight.repeat(num_patterns, 1)  # nn.Embedding(num_queries, query_dim) torch.Size([300, 4]) -> torch.Size([300, 4])
    if training:
        ...
        padding_label = torch.zeros(pad_size, hidden_dim).cuda()  # all zeros are padded.
        padding_bbox = torch.zeros(pad_size, 4).cuda()
        input_query_label = torch.cat([padding_label, tgt], dim=0).repeat(batch_size, 1, 1)  # torch.Size([2, 320, 256])
