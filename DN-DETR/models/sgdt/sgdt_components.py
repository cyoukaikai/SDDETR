...
def is_with_sgdt_layer(encoder_layer_config):
    if encoder_layer_config is None:
        return False
    return encoder_layer_config.find('sgdt') > -1
...
def split_encoder_layer(encoder_layer_config):
    ...
    for (l_type, num_l) in encoder_layer_list:
        if is_str_exist(l_type, 'sgdt'):
            sgdt_layer_ids.extend(list(range(cnt, cnt + num_l)))
        else:
            normal_layer_ids.extend(list(range(cnt, cnt + num_l)))
            cnt += num_l
    return normal_layer_ids, sgdt_layer_ids
