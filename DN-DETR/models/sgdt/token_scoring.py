...
from models.sgdt.topk import PerturbedTopKFunction, HardTopK, min_max_norm, \
...
def sgdt_token2feat(x, feat_map_size, mask=None):
    """
    N, B, C ->  B, C, h, w
    Args:
        x: N, B, C
...
class TokenScoringConv(nn.Module):
    ...
    def forward(self, input_x, feat_map_size, with_global_feat=False, mask=None):
        x = sgdt_token2feat(input_x, feat_map_size, mask=mask)  # torch.Size([2, 256, 30, 32])

        for conv in self.convs:
            x = conv(x)

        out = self.conv_logits(x)
        out = sgdt_feat2token(out)  # torch.Size([960, 2, 1])
