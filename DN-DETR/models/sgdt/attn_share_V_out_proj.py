...
class MultiheadAttentionShareVOutProj(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        if not self._qkv_same_embed_dim:
            ...
            self.in_proj_weight = Parameter(torch.empty((5 * embed_dim, embed_dim), **factory_kwargs))
            ...
            self.register_parameter('q_teacher_proj_weight', None)
        if bias:
            # self.in_proj_bias = Parameter(torch.empty(3 * embed_dim, **factory_kwargs))
            self.in_proj_bias = Parameter(torch.empty(5 * embed_dim, **factory_kwargs))
        else:
    ...
    def _reset_parameters(self):
        if self._qkv_same_embed_dim:
            ...
            xavier_uniform_(self.q_teacher_proj_weight)
    ...
    Outputs:
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights, attn_output_weights_logits = multi_head_attention_forward(
                query, key, value, query_teacher, key_teacher, self.embed_dim, self.num_heads,
...
def _in_projection_packed(
    ...
    if k is v:  # will not go to this branch
        ...
        w_q, w_k, w_v, w_q_teacher, w_k_teacher = w.chunk(5)
        if b is None:
            # b_q = b_k = b_v = None
            b_q = b_k = b_v = b_q_teacher = b_k_teacher = None
        else:
            # b_q, b_k, b_v = b.chunk(3)
            b_q, b_k, b_v, b_q_teacher, b_k_teacher = b.chunk(5)
        ...
        # Modification here
        # if freeze_wq or freeze_wk or freeze_wv or freeze_wq_teacher or freeze_wk_teacher:
        Q = linear_(freeze_wq, q, w_q, b_q)
        K = linear_(freeze_wk, k, w_k, b_k)
        V = linear_(freeze_wv, v, w_v, b_v)
        Q_teacher = linear_(freeze_wq_teacher, q_teacher, w_q_teacher, b_q_teacher)
        K_teacher = linear_(freeze_wk_teacher, k_teacher, w_k_teacher, b_k_teacher)
...
def multi_head_attention_forward(
    ...
    if not use_separate_proj_weight:
        ...
        q, k, v, q_teacher, k_teacher = _in_projection(query, key, value, query_teacher, key_teacher,
