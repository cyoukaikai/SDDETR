...
class TokenPredLoss:
    ...
    def cal_focal_loss_and_accu(self, pred_logits, token_targets, valid_tokens, pos_mask=None,
        ...
        loss = dict(
            sgdt_loss_fg=loss_,  #
            sgdt_fg_bg_accu=accu,  # pred_logits.new_tensor(accu)
        )
        return loss
...
class TokenScoringLoss(TokenPredLoss):
    ...
    def cal_loss(self, sgdt_output, sgdt_targets):
        return self.loss_func(sgdt_output, sgdt_targets)

    def loss_scale_score_reg_sigmoid_l1(self, sgdt_output, sgdt_targets, over_weigh_fg_loss=False,
                                        # pos_weight_ratio=1.0
                                        ):
        """
        ...
        valid_tokens_float = sgdt_output['valid_tokens']  # valid_tokens:  (N, B), 1 valid, 0, invalid
        valid_token_mask = sgdt_output['valid_tokens'].bool().reshape(-1)

        # logits to range [0, 1]
        pred_logits = torch.sigmoid(sgdt_output['small_scale_score_logit'])  # N, B
        targets = sgdt_targets['scale_gt']  # N, B
        ...
        loss = dict(
            # sgdt_loss_fg=fg_loss * self.sgdt_loss_weight,  # ,
            # sgdt_loss_small_scale=small_scale_loss * self.sgdt_loss_weight,  # ,
            sgdt_loss_token_significance=loss_token_sig,
            sgdt_fg_bg_accu=pred_logits.new_tensor(fg_bg_accu),  # pred_logits.new_tensor(fg_bg_accu)
            sgdt_small_scale_order_error=small_scale_order_errors,
        )
        token_update = self._extract_token_update_info(sgdt_output)
        loss.update(token_update)
        return loss

    def loss_fg_weighted_ce(self, sgdt_output, sgdt_targets):
        fg_loss, fg_accu, num_false_bg, num_fg_tokens, num_false_bg_custom_thd = self._cal_ce_loss(
            pred_logits=sgdt_output['fg_score_logit'],
            targets=sgdt_targets['fg_gt'],
            valid_tokens=sgdt_output['valid_tokens'].float(),
            soft_label=False,
            pos_weight_ratio=10,
            fg_or_split_token_labels=1 - sgdt_output['tokens_to_discard'],  # transfer to fg labels
        )

        # down weight the loss of fg_loss, small_scale_loss
        loss = dict(
            sgdt_loss_fg=fg_loss,  # ,  * self.sgdt_loss_weight
            sgdt_fg_accu=fg_loss.new_tensor(fg_accu),
            num_fg_tokens=num_fg_tokens,
            num_false_bg=num_false_bg,
            num_false_bg_custom_thd=num_false_bg_custom_thd,
            # sgdt_loss_small_scale=small_scale_loss,  # ,  * self.sgdt_loss_weight
            # sgdt_small_scale_accu=small_scale_loss.new_tensor(small_scale_accu),
            sgdt_num_tokens_to_split_remove=sgdt_output['tokens_to_discard'].sum(dim=0).float().mean(),
            sgdt_num_tokens_to_discard_original=sgdt_output['tokens_to_discard_original'].sum(dim=0).float().mean(),
            sgdt_num_tokens_to_split_original=sgdt_output['tokens_to_split_original'].sum(dim=0).float().mean(),
        )
        return loss

    @staticmethod
    def _extract_token_update_info(sgdt_output):
        out_dict = dict(
            sgdt_num_tokens_to_remove=sgdt_output['tokens_to_discard'].sum(dim=0).float().mean(),
            # sgdt_num_tokens_to_split=sgdt_output['tokens_to_split'].sum(dim=0).float().mean(),
            sgdt_num_tokens_to_discard_original=sgdt_output['tokens_to_discard_original'].sum(dim=0).float().mean(),
            sgdt_num_tokens_to_split_original=sgdt_output['tokens_to_split_original'].sum(dim=0).float().mean(),
        )
        return out_dict

    def loss_fg_scale_fake_for_gt(self, sgdt_output, sgdt_targets):
        """
        ...
        #     pred_logits=sgdt_output['fg_score_logit'],  # torch.Size([775, 4, 2])
        #     targets=sgdt_targets['fg_gt'],  # torch.Size([775, 4])
        #     valid_token_mask=valid_token_mask
        # )
        # small_scale_accu = self._cal_accu(
        #     pred_logits=sgdt_output['small_scale_score_logit'],
        #     targets=sgdt_targets['scale_gt'].bool(),  # float to bool
        #     valid_token_mask=valid_token_mask
        # )
        # loss = dict(
        #     sgdt_fg_accu=fg_accu,
        #     sgdt_small_scale_accu=small_scale_accu,
        # )
        loss = self._extract_token_update_info(sgdt_output)
        # loss.update(token_update)
        return loss

    # TODO: need to be adapted.
    def focal_loss_fg_scale_score(self, sgdt_output, sgdt_targets,
                                  alpha: float = 0.25, gamma: float = 2,
        ...
        if 'increase_resolution' in sgdt_output and sgdt_output['increase_resolution']:
            valid_tokens = sgdt_output['valid_tokens_original'].float()
        else:
            valid_tokens = sgdt_output['valid_tokens'].float()  # valid_tokens:  (N, B), 1 valid, 0, invalid
        ...
        if 'fg_score_logit' in sgdt_output:
        ...
        if 'small_scale_score_logit' in sgdt_output:
            if scale_fg_independent:
                small_scale_loss, small_scale_accu = cal_loss_accu(
                    pred_logits=sgdt_output['small_scale_score_logit'],
                    targets=sgdt_targets['scale_gt']
                )
            else:
                small_scale_loss, small_scale_accu = cal_loss_accu(
                    pred_logits=sgdt_output['small_scale_score_logit'],
                    targets=sgdt_targets['scale_gt'],
                    pos_mask=sgdt_targets['fg_gt']
                )
            loss.update(
                dict(
                    sgdt_loss_small_scale=small_scale_loss,
                    sgdt_small_scale_accu=small_scale_loss.new_tensor(small_scale_accu)
                )
            )
        token_update = self._extract_token_update_info(sgdt_output)
    ...
    def loss_fg_scale_score_ce(self, sgdt_output, sgdt_targets):
        """
        loss for a single encoder sgdt layer for scoring supervision of fg and scale prediction.
        Args:
            # fg_score, scale_score  # N, B, C, where C is the number of classes,
