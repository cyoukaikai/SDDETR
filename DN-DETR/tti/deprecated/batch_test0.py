...
token_scoring_gt_criterion=fg_scale_class_all_fg
    ...
    results = {}
    results_list = []
    min_split_scores = np.linspace(0.6, 0.9, 4)
    nms_thds = [0.5]
    ...
    result_file = result_dir + f'_use_proposal_with_debug{str(int(with_debug))}-grid-search.txt'
    smrc.utils.save_multi_dimension_list_to_file(result_file, list_to_save=results_list)
...
token_scoring_gt_criterion=fg_scale_class_all_fg
    ...
    results = {}
    results_list = []
    min_split_scores = np.linspace(0.6, 0.9, 4)  # 0.2, 0.5, 4
    nms_thds = [0.5]
