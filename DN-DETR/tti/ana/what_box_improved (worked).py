...
RESULT_DIR = os.path.join(LIB_ROOT_DIR, 'visualize/ana_what_object_improved/v0')
...
def get_eval_result(input_dir):
    coco_res_ = load_and_merge_result(input_dir)
    return eval(coco_res_)
...
coco_eval_detr_original = get_eval_result(original_detr_pred_dir_path)

# coco_eval_detr_original = torch.load(original_detr_pred_file)
# coco_eval_improved = torch.load(pred_file)
# ana_improved_det(cocoEval=coco_eval_detr_original, cocoEvalImproved=coco_eval_improved)
improved_pred_dir_path = os.path.join(RESULT_DIR, 'eval_results_batchsize_2')  # original_detr_coco_eval
coco_eval_improved = get_eval_result(improved_pred_dir_path)
    ...
    for i in range(len(gt_matches0)):
        ...
        if det_id0 == 0 and det_id1 == 0:
            ...
            if iou0 > iou1:
                ...
                if rec in det_ana_res['improved'][imgId]:
                    print(f'WHy')
                ...
                if rec in det_ana_res['worse'][imgId]:
                    print(f'WHy')
...
save_path = os.path.join(RESULT_DIR, 'compared_to_batchsize2.pkl')
