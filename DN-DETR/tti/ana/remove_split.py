...
class RemoveAnalysis:
    def __init__(self):
        self.out_dir = os.path.join(checkpoint_dir, 'remove_ana_results')

    def easy_remove_ana(self, with_debug=True):
        """
        Does removing easy bg tokens increase the accuracy?
        Intuition: No.

        Easy bg, from proposal, by from token scoring.
        Args:
            with_debug:
        Returns:
        """
        ...
        token_scoring_discard_split_criterion = [
            '--token_scoring_discard_split_criterion',
            f'gt_only_exp-no_split-debug_gt_remove_ratio0.5'
        ]
        # -use_proposal-proposal_remove_gt_filter
        results_list = []
        ...
        for use_conf_score in [False]:  # '', '-use_conf_score' , True
            for nms_thd in nms_thds:
                for min_fg_score in min_fg_scores[::-1]:
                    print(f'------------------------------------------------- \n'
                          f'min_fg_score = {min_fg_score} \n {token_scoring_discard_split_criterion} \n'
                          f'----------------------------------------------------\n')
                    proposal_scoring = f'min_fg_score{min_fg_score}-nms_thd{nms_thd}'
                    if use_conf_score:
                    ...
                    results_list.append([use_conf_score, nms_thd, min_fg_score] + coco_eval_bbox)
                    print(f'{results_list[-1]}')

        # 44.7
        result_file = result_dir + f'/use_proposal_with_debug{str(int(with_debug))}-' \
